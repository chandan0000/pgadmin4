...
class DatabaseView(PGChildNodeView):
    ...
    def retrieve_last_system_oid(self):
        return (
            ...
            else self._DATABASE_LAST_SYSTEM_OID
        )

    def get_icon(self, res, connected):
        if not connected and not res['is_template']:
            return "icon-database-not-connected"
        elif not connected:
            return 'icon-database-template-not-connected'
        elif res['is_template']:
            return 'icon-database-template-connected'
        else:
    ...
    def skip_db(self, row):
        ...
        return bool(
            not self.blueprint.show_database_template
            and row['is_template']
            and not row['is_sys_obj']
            and row['name'] not in ('postgres', 'edb')
        )
